...
tools<-c("PTP","MiXCR", "changeo","scoper_ID","scoper_hierarchical","scoper_spectral")
...
get_MSE_median_fam_size<-function(data,df) {
  ...
  mse<-mean((as.numeric(real$real_med_size) - as.numeric(pred$median_size_families))^2)
  return(mse)
}
...
for (x in clonal_families) {
  ...
  number_families<-tidyr::expand_grid(tools,clonal_families,SHM, leaves,balance)
