...
get_values_scoper<-function(dpath,file_name) {
  ...
  number_fams <- length(scoper_sum$n)
  med_fam_size <- median(scoper_sum$n)
...
old<-read.table("/home1/kavoss/panfs/simulations/output_w_singletons.csv",sep=",",header=TRUE, fill=TRUE, row.names=NULL)

combined<-rbindlist(list(old,combined))
combined$clones<-as.character(combined$clones)
write.csv(combined, "/home1/kavoss/panfs/simulations/output_w_scoper_singletons.csv", row.names=FALSE)
